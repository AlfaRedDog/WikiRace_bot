...
//    }
    ...
    @KafkaListener(
        ...
        properties = ["key.deserializer=org.apache.kafka.common.serialization.StringDeserializer", "value.deserializer=com.itmo.services.kafka.models.AuthRequestMessageDeserializer", "value.deserializer.type=com.itmo.services.kafka.models.AuthRequestMessage"]
    )
    fun consumeFromSubscribe(message: AuthRequestMessage) {
        ...
        kotlin.runCatching { tokenManager.readAccessToken(message.token) }.onSuccess { user ->
            ...
            responseMessage = AuthResponseMessage(ResponseStatusEnum.ACCESS, "default", "default")
            messageProducer.produceMessageAuthResponse(
                responseMessage, KafkaConfig.Subscribe_topic + "-${message.authId}"
            )
        }
